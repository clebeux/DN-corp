...
function afficheRequete_debug($sql) {
...
function afficheListe_SQL($sql, $ID_Champ, $Nom_Champ){
    $results = tableSQL($sql);
    
    echo "<SELECT name='afficheListe_SQL'>";
    foreach ($results as $ligne) {
        echo "<OPTION value = '$ligne[$ID_Champ]'>$ligne[$Nom_Champ]</OPTION>";
    }
    echo "</SELECT>";
